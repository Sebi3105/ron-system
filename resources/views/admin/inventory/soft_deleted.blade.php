<x-app-layout>
    <div class="container mx-auto mt-5">
        ...
        <h1 class="text-2xl font-semibold mb-4 text-center text-white">Soft Deleted Inventory Items</h1>
        <div class="py-4 overflow-auto max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="p-4 sm:p-8 bg-gray-200 shadow sm:rounded-lg">
                <table id="softDeletedInventoryTable" class="min-w-full table-fixed bg-gray-200 text-black border border-gray-400">
                    ...
                    <tbody>
                        @foreach($softDeletedItems as $item)
                            <tr class="hover:bg-gray-400">
                                ...
                                <td class="px-4 py-2 border-b border-gray-400 text-gray-800">{{ $item->product_name }}</td>
                                ...
                                <td class="px-4 py-2 border-b border-gray-400 text-gray-800">
                                    <form action="{{ route('admin.inventory.restore', $item->product_id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to restore this item?');">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="bg-green-500 hover:bg-green-600 text-white font-bold py-1 px-2 rounded">Restore</button>
                                    </form>

                                    <form action="{{ route('admin.inventory.forceDelete', $item->product_id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this item permanently?');">
                                        @csrf
                                        @method('DELETE')
    ...
    <script>
        $(document).ready(function() {
            $('#softDeletedInventoryTable').DataTable({
