<x-app-layout>
    <div class="container mx-auto mt-5">
        ...
        <div class="py-4 overflow-auto max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="p-4 sm:p-8 bg-gray-200 shadow sm:rounded-lg">
                <table id="softDeletedCustomerTable" class="min-w-full table-fixed bg-gray-200 text-black border border-gray-400">
                    <thead class="bg-gray-300">
                        <tr>
                            <th class="px-4 py-2 border-b border-gray-400 text-gray-800">#</th>
                            <th class="px-4 py-2 border-b border-gray-400 text-gray-800">Name</th>
                            <th class="px-4 py-2 border-b border-gray-400 text-gray-800">Address</th>
                            <th class="px-4 py-2 border-b border-gray-400 text-gray-800">Contact Number</th>
                    ...
                    <tbody>
                        @foreach($softDeletedItems as $item)
                            <tr class="hover:bg-gray-400">
                                ...
                                <td class="px-4 py-2 border-b border-gray-400 text-gray-800"> {{ $item->name }}</td> <!-- Accessing the related inventory's product name -->
                                <td class="px-4 py-2 border-b border-gray-400 text-gray-800">{{ $item->address }}</td>
                                <td class="px-4 py-2 border-b border-gray-400 text-gray-800">{{ $item->contact_no }}</td>
                                ...
                                <td class="px-4 py-2 border-b border-gray-400 text-gray-800">
                                    <form action="{{ route('admin.customer.restore', $item->customer_id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to restore this item?');">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="bg-green-500 hover:bg-green-600 text-white font-bold py-1 px-2 rounded">Restore</button>
                                    </form>

                                    <form action="{{ route('admin.customer.forceDelete', $item->customer_id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this item permanently?');">
                                        @csrf
                                        @method('DELETE')
    ...
    <script>
        $(document).ready(function() {
            $('#softDeletedCustomerTable').DataTable({
