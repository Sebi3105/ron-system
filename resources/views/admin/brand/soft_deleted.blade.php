                ...
                <table id="softDeletedBrandTable" class="min-w-full table-fixed bg-gray-200 text-black border border-gray-400">
                    <thead class="bg-gray-300">
                        <tr>
                            ...
                            <th class="px-4 py-2 border-b border-gray-400 text-gray-800">Brand Name</th>
                    ...
                    <tbody>
                        @foreach($softDeletedItems as $item)
                            <tr class="hover:bg-gray-400">
                                ...
                                <td class="px-4 py-2 border-b border-gray-400 text-gray-800"> {{ $item->brand_name }}</td> <!-- Accessing the related inventory's product name -->
                                ...
                                <td class="px-4 py-2 border-b border-gray-400 text-gray-800">
                                    <form action="{{ route('admin.brand.restore', $item->brand_id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to restore this item?');">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="bg-green-500 hover:bg-green-600 text-white font-bold py-1 px-2 rounded">Restore</button>
                                    </form>

                                    <form action="{{ route('admin.brand.forceDelete', $item->brand_id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this item permanently?');">
                                        @csrf
                                        @method('DELETE')
