        ...
        <div class="flex-1 md:ml-48 lg:ml-64 mt-0 bg-gray-100 text-gray-800">
            ...
            <style>
                ...
                .table {
                    ...
                    /* Magkadikit ang linya */
                    border: 1px solid #ddd;
                    /* Linya sa paligid ng table */
                }
                        ...
                        <table id="serials">
                            ...
                            <tbody>
                                ...
                                <tr class="border-b hover:bg-gray-100">
                                    ...
                                    <td class="px-4 py-2 ">{{ $item->inventory->product_name ?? 'N/A' }}</td>
                                    <td class="px-4 py-2 ">{{ $item->inventoryitem->serial_number ?? 'N/A' }}</td>
                                    <td class="px-4 py-2">{{ $item->amount }}</td>
                                    <td class="px-4 py-2 ">{{ $item->payment_method }}</td>
                                    <td class="px-4 py-2 ">{{ $item->payment_type }}</td>
                                    <td class="px-4 py-2 ">{{ $item->deleted_at }}</td>
                                    <td class="px-4 py-2 flex gap-2">
                                        <!-- Restore Button -->
                                        <form action="{{ route('admin.sales.restore', $item->sales_id) }}" id="categorydel" method="POST" style="display:inline;">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="bg-green-500 hover:bg-green-600 text-white py-1 px- rounded" id="saveCategoryButton">
                                                Restore
                                            </button>
                                        </form>
                                        <!-- Delete Permanently Button -->
                                        <form action="{{ route('admin.sales.forceDelete', $item->sales_id) }}" id="categorydel" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" id="cancelconfirmationModal" class="bg-red-500 hover:bg-red-600 text-white py-1 px-2 rounded">
                                                Delete
                                            </button>
                                        </form>
                                    </td>
        ...
        <div id="cancelModal" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 hidden">
            <div class="bg-white max-w-sm w-full rounded-md shadow-lg">
                ...
                <div class="flex justify-center gap-4">
                    ...
                    <a href="{{ route('admin.sales.soft_deleted') }}" id="confirmCancel" class="px-6 py-3 bg-gradient-to-r from-red-500 to-red-700 text-white rounded-md hover:from-red-600 hover:to-red-800 transition">
                        Confirm
                    </a>
                </div>
            </div>
