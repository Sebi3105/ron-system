    ...
    <div class="flex flex-col md:flex-row h-screen">
        ...
        <div class="flex-1 md:ml-48 lg:ml-64 mt-0 bg-gray-100 text-gray-800">
            ...
            <header class="bg-gray-200 py-3 px-3 md:px-6 fixed top-0 md:left-48 lg:left-64 right-0 z-20 h-16 flex items-center justify-between text-black shadow-md">
                <h1 class="text-lg font-bold">Technician Profile</h1>
            </header>
                    ...
                    <div class="table">
                        <table id="serials">
                            ...
                            <tbody>
                                ...
                                <td class="px-4 py-2 "> {{ $item->name }}</td> <!-- Accessing the related inventory's product name -->
                                <td class="px-4 py-2">{{ $item->contact_no }}</td>
                                <td class="px-4 py-2 ">{{ $item->deleted_at }}</td>
                                    <td class="px-4 py-2 flex gap-2">
                                        <!-- Restore Button -->
                                        <form action="{{ route('admin.techprofile.restore', $item->technician_id) }}" id="categorydel" method="POST" style="display:inline;">
                                            @csrf
                                            @method('PATCH')
                                        ...
                                        <form action="{{ route('admin.techprofile.forceDelete', $item->technician_id) }}" id="categorydel" method="POST"  style="display:inline;">
                                            @csrf
                                            @method('DELETE')
